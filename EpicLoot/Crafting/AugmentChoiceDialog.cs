    ...
    public class AugmentChoiceDialog : MonoBehaviour
    {
        ...
        public void Show(AugmentTabController.AugmentRecipe recipe, Action<AugmentTabController.AugmentRecipe, MagicItemEffect> onCompleteCallback)
        {
            ...
            for (var index = 0; index < newEffectOptions.Count; index++)
            {
                ...
                text.text = Localization.instance.Localize((index == 0 ? "<color=white>($mod_epicloot_augment_keep)</color> " : "") + MagicItem.GetEffectText(effect, rarity, true, magicItem.LegendaryID));
