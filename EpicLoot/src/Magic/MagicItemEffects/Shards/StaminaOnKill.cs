...
namespace EpicLoot.MagicItemEffects.Shards {
    ...
    public static class StaminaOnKill {
        ...
        public static void OnDamageDealt(Character __instance, HitData hit, Character attacker) {
            ...
            // Kill check via IsLethalHit: against a remote-owned victim the RPC carrying this hit has not
            // executed yet, so the post-hit health is estimated; against one this client owns, it has.
            if (hit == null || player == null || __instance == player || attacker != player
                || __instance.IsPlayer() || __instance.IsTamed()
                || !MagicEffectsHelper.IsLethalHit(__instance, hit)) {
                return;
            }
