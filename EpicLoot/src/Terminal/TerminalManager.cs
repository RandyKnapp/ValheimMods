...
public static partial class TerminalManager
{
    ...
    public static class Register_Commands
    {
        public static void Postfix()
        {
            ...
            _ = new Command("resetbounties", "removes active bounties", ResetBounties);
            _ = new Command("testbountynames", "print randomly generated bounty names: [amount]", TestBountyNames);
            _ = new Command("resetadventure", "clear player adventure data", ResetAdventure);
            _ = new Command("adventureindex", "print or rebuild the world biome index: [biome|rebuild]", AdventureIndexInfo);
            _ = new Command("bounties", "print available bounties: [interval]", PrintAvailableBounties);
            _ = new Command("biomes", "print biome progression order, boss keys and status", PrintBiomes);
            _ = new Command("knowbiome", "mark a biome as discovered by the local player: [biome]", KnowBiome, GetKnowBiomeOptions);
            _ = new Command("playerbounties", "print player available bounties", PrintPlayerAvailableBounties);
            ...
            _ = new Command("lucktest", "rolls an example loot table with the sepcified luck eg: lucktest Greydwarf 1.0", TestLuck, GetTestLuckOptions);
            _ = new Command("fishlucktest", "samples the lucky fishing treasure roll: [effectValue] [samples]", TestFishingLuck);
            ...
            _ = new Command("magicapi", "exercise the public API: version | query | providers | events | roll [rarity]", ApiDiagnostics, GetApiDiagnosticsOptions);
