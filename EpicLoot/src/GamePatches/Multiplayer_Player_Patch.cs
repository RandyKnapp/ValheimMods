...
namespace EpicLoot
{
    public static class Multiplayer_Player_Patch
    {
        public static void UpdatePlayerZDOForEquipment(Player player, ItemDrop.ItemData item, bool equip)
        {
            ...
            string data = equip ? (magicItem != null && magicItem.IsUniqueLegendary() ? magicItem.LegendaryID : "") : "";
            ...
            switch (itemType)
            {
                ...
                case ItemDrop.ItemData.ItemType.Shield:
                    ...
                    break;
                case ItemDrop.ItemData.ItemType.Chest: zdo.Set("ch-ell", data);
                    break;
                case ItemDrop.ItemData.ItemType.Legs: zdo.Set("lg-ell", data);
                    break;
                case ItemDrop.ItemData.ItemType.Helmet: zdo.Set("hl-ell", data);
                    break;
                case ItemDrop.ItemData.ItemType.Shoulder: zdo.Set("sh-ell", data);
                    break;
                case ItemDrop.ItemData.ItemType.Utility: zdo.Set("ut-ell", data);
                    break;
        ...
        public static class WatchLegendaryEquipment_Player_Update_Patch
        {
            ...
            private static bool DoCheck(Player player, ZDO zdo, string equipKey,
                string legendaryDataKey, ref ItemDrop.ItemData itemData)
            {
                string zdoLegendaryID = zdo.GetString(legendaryDataKey);

                if (string.IsNullOrEmpty(zdoLegendaryID))
                {
                    bool hadItem = itemData != null;
                    if (hadItem)
                ...
                string currentLegendaryID = itemData?.GetMagicItem()?.LegendaryID;
                if (currentLegendaryID == zdoLegendaryID)
                    return false;
                int itemHash = zdo.GetInt(equipKey);
            ...
            public static void ForceResetVisEquipment(Humanoid humanoid, ItemDrop.ItemData item)
            {
                ...
                {
                    case ItemDrop.ItemData.ItemType.OneHandedWeapon:
                        if (humanoid.m_rightItem != null &&
                            humanoid.m_rightItem.m_shared.m_itemType == ItemDrop.ItemData.ItemType.Torch &&
        ...
        public static class WatchMultiplayerMagicEffects_Player_Patch
        {
            ...
            {
                if (player == Player.m_localPlayer && player.m_nview != null && player.m_nview.GetZDO() is ZDO zdo)
                {
