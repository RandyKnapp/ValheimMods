...
#pragma warning restore CS0618
    ...
    public static float GetTotalActiveMagicEffectValue(Player player, ItemDrop.ItemData item, string effectType,
        float scale = 1.0f) {
        object[] result = API_GetTotalActiveMagicEffectValue.Invoke(player, item, effectType, scale);
    ...
    /// <param name="effectValue">the effect's total value, 0 when absent</param>
    /// <returns>true if the player or the item has the effect</returns>
    [PublicAPI]
    public static bool HasActiveMagicEffect(Player player, string effectType, ItemDrop.ItemData item, out float effectValue) {
        effectValue = 0f;
        object[] result = API_HasActiveMagicEffect.Invoke(player, effectType, item, effectValue);
        effectValue = (float)(result[4] ?? 0f);
        bool output = (bool)(result[0] ?? false);
        logger.LogDebug($"Has active effect: {effectType}, {output}, value: {effectValue}");
        return output;
    }
